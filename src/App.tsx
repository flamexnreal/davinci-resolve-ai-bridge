...
const consoleCommands: Record<OS, string> = {
  mac: 'exec(open("/Users/YOUR_NAME/.resolve-ai-bridge/ResolveConsole.py", encoding="utf-8").read())',
  windows:
...
const configExample = `{
  "mcpServers": {
    "resolve-ai-bridge": {
      ...
      "env": {
        "RESOLVE_AI_BRIDGE_TOKEN": "RAB_TOKEN_FROM_RESOLVE_CONSOLE"
      }
    }
  }
}`;
...
const providers: Record<Provider, { title: string; steps: string[] }> = {
  Antigravity: {
    ...
    steps: [
      ...
      "Merge the resolve-ai-bridge entry printed in the Resolve Console into the existing mcpServers object. Keep any servers already there.",
  ...
  "Claude Code": {
    ...
    steps: [
      "Copy the CLAUDE CODE COMMAND printed in the Resolve Console. The installer also saves it at ~/.resolve-ai-bridge/claude-command.txt.",
  ...
  Codex: {
    ...
    steps: [
      "Copy the CODEX CLI COMMAND printed in the Resolve Console. The installer also saves it at ~/.resolve-ai-bridge/codex-command.txt.",
  ...
  Cursor: {
    ...
    steps: [
      ...
      "Merge the resolve-ai-bridge entry printed in the Resolve Console into mcpServers.",
      "Save the JSON and switch the server on. A green status means Cursor started the local Python server.",
      "Keep Resolve open and keep the Console-started bridge running while you edit.",
    ],
  },
};
...
const faq = [
  {
    ...
    a: "It is a real local shared secret. The MCP server includes it in every file-queue request, and the Resolve agent rejects requests with the wrong token. It is not an internet account or a paid key. Keep it private, and rotate it with the installer if it is exposed.",
  },
  {
    q: "Can I rename the downloaded GitHub folder?",
    a: "Yes. The installer copies the runtime to a fixed folder in your home directory. Do not rename ResolveConsole.py inside ~/.resolve-ai-bridge after installation. If anything was moved, run install.py again and use the newly printed Console line.",
  },
  {
    q: "Does Remotion control Resolve?",
    a: "No. Remotion creates polished motion graphics and complete video clips with React. The bridge then imports or appends the rendered file to Resolve. This two-part workflow is often better than asking a model to invent complex motion directly through Resolve's limited scripting API.",
  },
  {
    q: "What should I do if my AI provider looks different?",
    a: "Use the provider's MCP settings and add a local stdio server. The three values that matter are command, args, and RESOLVE_AI_BRIDGE_TOKEN. The Resolve Console prints all three. You can also give your provider the help prompt near the end of this guide.",
  },
];
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#f5f2ea] text-slate-900">
        ...
        <div id="top" className="relative z-10 mx-auto flex h-full max-w-[1440px] items-end px-6 pb-14 pt-28 sm:px-10 sm:pb-20 lg:px-16 lg:pb-24">
          <div className="max-w-4xl">
            ...
            <motion.p initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.66, duration: 0.7 }} className="mt-8 max-w-xl text-base leading-7 text-slate-100 sm:text-xl sm:leading-8">
              A local MCP bridge that lets Claude, Codex, Antigravity, and other compatible AI tools work with the project open in Resolve.
            </motion.p>
      ...
      <section id="how" className="page-section py-24 sm:py-32">
        <Reveal>
          <SectionIntro number="00 / Read this first" title="One bridge. Three local parts." text="Nothing is uploaded to a bridge service. Resolve, the file queue, and your AI client's MCP process stay on your computer." />
        </Reveal>
        <Reveal className="mt-16 grid gap-0 border-y border-slate-300 md:grid-cols-3">
          {[
            ["01", "Resolve Console", "Starts a small background worker inside the Resolve session you already have open."],
            ["02", "Local queue", "Moves authenticated JSON requests through ~/.resolve-ai-bridge. It does not open a network port."],
            ["03", "MCP server", "Gives your AI clear editing tools and waits for Resolve to return the result."],
          ].map(([n, title, text], i) => (
        ...
        <Reveal className="mt-10 flex items-start gap-4 bg-[#e8edf5] px-5 py-5 sm:px-7">
          ...
          <p className="max-w-4xl text-sm leading-6 text-slate-700"><strong>Important:</strong> this project uses Resolve's internal Python Console because external scripting is restricted in some editions. Resolve versions and operating systems vary, so the included doctor check is the source of truth on your machine. The web guide builds successfully; real Resolve integration still needs to be verified on the Resolve version you use.</p>
        </Reveal>
      </section>
          ...
          <Reveal className="mt-14 section-grid">
            ...
            <div className="max-w-3xl">
              <ol className="divide-y divide-slate-200 border-y border-slate-200">
                ...
                <li className="step-row"><span>3</span><div><h3>Run the installer</h3><p>It creates a private runtime at <code>~/.resolve-ai-bridge</code>, a Python virtual environment, a persistent token, and a ready-to-copy MCP configuration.</p></div></li>
              </ol>
              ...
              <p className="mt-4 text-sm leading-6 text-slate-600">Wait for <strong>INSTALL COMPLETE</strong>. If Python asks for permission or a firewall prompt appears, the bridge itself does not need an inbound network port. The installer only downloads the Python MCP dependency.</p>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section id="connect" className="page-section py-24 sm:py-32">
        <Reveal>
          <SectionIntro number="02 / Start Resolve" title="Paste one line in the Python 3 Console." text="The line starts a daemon thread and returns control to Resolve immediately. Keep Resolve open while your AI is editing." />
        </Reveal>
        <Reveal className="mt-14 section-grid">
          <div className="space-y-3 text-sm leading-6 text-slate-600">
            <p className="section-number">Inside Resolve</p>
            <p><strong className="text-slate-900">1.</strong> Open your project and timeline.</p>
            <p><strong className="text-slate-900">2.</strong> Choose Workspace, then Console.</p>
            <p><strong className="text-slate-900">3.</strong> Select <strong>Py3</strong> in the Console language menu.</p>
            <p><strong className="text-slate-900">4.</strong> Replace YOUR_NAME below, paste the whole line, and press Enter.</p>
          </div>
          <div className="min-w-0 max-w-3xl">
            <CopyBlock code={consoleCommands[os]} label="Resolve Python 3 Console" />
            <div className="mt-5 border-l-2 border-blue-700 pl-5 text-sm leading-6 text-slate-600">
              <p><strong className="text-slate-950">Use the installed filename exactly:</strong> <code>ResolveConsole.py</code>. The downloaded GitHub folder can be renamed; this installed file should not be renamed. Re-run the installer if the path changes.</p>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal className="mt-20 section-grid border-t border-slate-300 pt-12">
          ...
          <div className="max-w-3xl">
            <h3 className="text-2xl font-semibold tracking-[-0.025em]">The Console prints the token and MCP server lines next.</h3>
            <p className="mt-3 leading-7 text-slate-600">Copy the complete configuration printed by Resolve. It already contains the correct absolute Python path, server path, and your token. The shape looks like this:</p>
            <div className="mt-6"><CopyBlock code={configExample} label="Example only - use the filled version printed in Resolve" compact /></div>
            <p className="mt-4 text-sm leading-6 text-slate-600">The installer also saves the same filled JSON at <code>~/.resolve-ai-bridge/mcp-config.json</code>. You do not need to type the token by hand.</p>
          </div>
        </Reveal>
      </section>
      ...
      <section className="page-section py-24 sm:py-32">
        <Reveal>
          <SectionIntro number="04 / Prove it works" title="Test before asking for a real edit." text="A one-minute check prevents the model from guessing when Resolve is offline or the wrong timeline is open." />
        </Reveal>
        <Reveal className="mt-14 grid gap-10 lg:grid-cols-[1fr_1.3fr]">
          <div className="space-y-7">
            {[
              ...
              ["Ask for status", "Call resolve_status and tell me the open project and timeline. Do not edit anything."],
      ...
      <section id="remotion" className="bg-[#f0e7d8] py-24 sm:py-32">
        <div className="page-section">
          <Reveal>
            <SectionIntro number="05 / Heavily recommended" title="Use Remotion for stronger motion design." text="Resolve scripting is useful for timeline organization and repeatable edits. Remotion gives your coding agent a much better language for designed titles, explainers, product scenes, and precise animation." />
          </Reveal>
          <Reveal className="mt-14 section-grid">
      ...
      <section id="help" className="page-section py-24 sm:py-32">
        <Reveal>
          <SectionIntro number="06 / Help" title="Common questions, plain answers." text="Start with the exact error, not a reinstall. The queue, token, MCP process, and Resolve agent can each be checked separately." />
        </Reveal>
        <Reveal className="mt-14 section-grid">
          <div>
            <p className="section-number">Troubleshooting order</p>
            <p className="mt-4 max-w-xs text-sm leading-6 text-slate-600">Run doctor, check the Resolve Console still says the bridge is running, then confirm the token in your MCP config matches the Console.</p>
          </div>
