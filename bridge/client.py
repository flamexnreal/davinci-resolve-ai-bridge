...
def _token():
    ...
    try:
        ...
        raise BridgeOffline(
            "Bridge token is missing. Run install.py, then use the MCP config printed in Resolve."
        ) from exc
...
def require_online():
    ...
    if data is None:
        raise BridgeOffline(
            "Resolve agent is offline. In Resolve open Workspace > Console, select Py3, and run the installed ResolveConsole.py line."
        )
    return data
